...
#this func sends an email every day at 9 am with the temp 
def watering(t):
	print "I am Waterring"
	to = 'scortimix.dc@gmail.com'
	gmail_user = 'scortimix.dc@gmail.com'
	gmail_pwd = 'd2011505'
	...
	header = 'To:' + to + '\n' + 'From: ' + gmail_user + '\n' + 'Subject:testing \n'
	print header
	msg = header + '\n this is  msg from your plants \n\n' + str( ((float( temmp)*1.8+32)))
	smtpserver.sendmail(gmail_user, to, msg)
	print 'done!'
	smtpserver.close()
	#turn on water for 5 seconds
...
#water yo plants dog
schedule.every().day.at("9:00").do(watering,'its 9:00')
...
while True:
	...
	if (config.DS3231_Present == True):
		...
		temmp = ds3231.getTemp()
	
		print "DS3231 Temperature= \t%0.2f C" % temmp
		print "DS3231 temperature=  \t%.2f F"  % (float( temmp)*1.8+32)
                GPIO.setup(4, GPIO.OUT)
                if ((float( temmp)*1.8+32) < 70):
	...
	if (config.BMP280_Present):
		...
		print 'mine  \t{0:0.2f} F'.format(bmp280.read_temperature()*9.0/5.0 + 32)
